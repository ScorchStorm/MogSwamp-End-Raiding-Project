...
import importlib
import matplotlib
# On some Linux setups (especially Wayland) matplotlib silently falls back to the
# non-interactive 'Agg' backend, which makes plt.show() a no-op instead of opening a window.
# matplotlib.use() only records a backend name without importing it, so we have to actually
# try importing each backend's underlying module to find one that really works here.
_backend_found = False
for _backend, _test_import in (
    ...
    try:
        importlib.import_module(_test_import)
...
if not _backend_found:
    raise RuntimeError(
        "No interactive matplotlib backend is available. Install one of: "
        "PyQt5/PyQt6/PySide2/PySide6 (for QtAgg), python3-gi + gir1.2-gtk-3.0 (for GTK3Agg), "
        "or python3-tk (for TkAgg)."
    )
...
waypoint_file = r'/home/Dumy/.var/app/org.prismlauncher.PrismLauncher/data/PrismLauncher/instances/1.21.11/minecraft/xaero/minimap/Multiplayer_play.flatnet.gg/dim%1/mw$default_1.txt'
...
# Set this to True only when you actually want raided cities marked in the server spreadsheet
write_to_spreadsheet = True

def add_deferred_pan_zoom(fig, ax):
    # Custom pan/zoom that freezes the view while dragging or scrolling, and only
    # redraws once (on mouse release, or once scrolling pauses briefly). Redrawing
    # ~16,000+ city markers on every single mouse-move/scroll-tick is what makes
    # continuous live panning/zooming feel laggy with this many points, so this
    # trades away the "smooth while dragging" feel for one fast redraw per action.
    # i still have no idea if it actually improves the performance tho
    state = {'panning': False, 'press_xy': None, 'orig_xlim': None, 'orig_ylim': None}
    ...
    def on_press(event):
        ...
        state['orig_ylim'] = ax.get_ylim()

    def on_release(event):
        if not state['panning']:
            return
        state['panning'] = False
        if event.xdata is None or event.ydata is None or state['press_xy'] is None:
            return
        dx = event.xdata - state['press_xy'][0]
        dy = event.ydata - state['press_xy'][1]
        x0, x1 = state['orig_xlim']
        y0, y1 = state['orig_ylim']
        ax.set_xlim(x0 - dx, x1 - dx)
        ax.set_ylim(y0 - dy, y1 - dy)
        fig.canvas.draw()

    zoom_state = {'timer': None}
    ...
    def on_scroll(event):
        ...
        # Debounce: rapid scroll-wheel ticks only trigger one redraw after a short pause,
        # instead of a full redraw on every single tick.
        if zoom_state['timer'] is not None:
            zoom_state['timer'].stop()
        timer = fig.canvas.new_timer(interval=120)
        timer.single_shot = True
        timer.add_callback(fig.canvas.draw)
        zoom_state['timer'] = timer
        timer.start()
...
def position_window_top_left(fig):
    # Best-effort attempt to move the plot window to the top-left corner of the screen.
    # Different backends expose this differently, and Wayland doesn't allow apps to
    # position their own windows at all, so any failure here is just silently ignored.
    manager = fig.canvas.manager
    try:
        manager.window.wm_geometry("+%d+%d" % (0, 0)) # TkAgg
        return
    except Exception:
        pass
    try:
        manager.window.move(0, 0) # Qt/GTK under X11
        return
    except Exception:
        pass
...
def choose_first_end_city():
    ...
    ax.scatter(unraided_cities_x, unraided_cities_z, s=7, c='black', picker=True, label='unraided_cities')
    ax.scatter(raided_cities_x, raided_cities_z, s=7, c='red', label = 'raided_cities')
    ax.plot([])
    ax.set(xlim=(min_x-border, max_x+border), ylim=(max_z+border, min_z-border))
    fig.canvas.draw()
    manager = fig.canvas.manager
    position_window_top_left(fig)
    ax2background = fig.canvas.copy_from_bbox(ax.bbox)
    add_deferred_pan_zoom(fig, ax)
...
def start_canvas(path_city_x, path_city_z, city_x, city_z, tour = []):
    ...
    ax.set(xlim=(min_x - border, max_x + border,), ylim=(max_z + border, min_z - border))
    position_window_top_left(fig) # move the figure to top right corner of the screen
